...
| 维度 | 定义 | 核心测试点 |
| :--- | :--- | :--- |
| **P1 (基础功能)** | 协议核心功能可用性 | 4种寄存器类型(Coils/Holding/...)读写、0x10指令支持、DataStore 状态一致性。 |
| **P2 (数据异常)** | 业务数据逻辑攻击 | `HONEYPOT` 数值溢出注入、`OVERSIZED` 超长报文导致主站缓冲区风险测试。 |
| **P3 (环境基建)** | 链路与时序异常注入 | `DROP` 连接随记拆除、`DELAY` 慢响应注入、`MAX_CONNS` 并发配额。 |
| **P4 (边界条件)** | 协议格式与规约越界 | `TRUNCATED` 报文负载截断、`MISMATCH` 事务 ID (TID) 篡改。 |
| **P5 (交付视角)** | 稳定性与组合场景验证 | `SOAK` 长时稳定性测试、`MIXED` 组合攻击 (超长+分片)。 |
