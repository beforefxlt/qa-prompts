...
- **原始资料层** (Raw Data Layer)：管理账户、成员与检查单原件/脱敏件。
...
### 2.1 `accounts` (账号表)
约束：MVP 支持单账户管理家庭（家庭空间即该账户所有从属）。

| 字段名 | 类型 | 约束 / 说明 |
| :--- | :--- | :--- |
| `id` | UUID | 主键 |
| `phone_or_email` | VARCHAR | 唯一索引，登录名 |
| `created_at` | TIMESTAMPTZ | 创建时间 |
| `updated_at` | TIMESTAMPTZ | 更新时间 |

### 2.2 `member_profiles` (成员档案表)
约束：采用软删除，保留历史引用。多成员共存。

| 字段名 | 类型 | 约束 / 说明 |
| :--- | :--- | :--- |
| `id` | UUID | 主键 |
| `account_id` | UUID | 外键，关联 `accounts.id`。索引 |
...
| `reviewer_id` | UUID | 外键，指向 `accounts.id`（核准人，MVP可与家庭账号融合） |
