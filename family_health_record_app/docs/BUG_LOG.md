...
## 📅 v2.9.0 成人成员详情页修复 (2026-04-05)

### [BUG-051] 成人成员详情页无数据趋势展示
- **现象**: 成人成员（`member_type='adult'`）的详情页只显示人名，没有数据趋势看板
- **根由**: 前端 `members/[id]/page.tsx` 只处理了 `child` 和 `senior` 两种类型，漏掉了 `adult`
- **修复**:
  - 前端 Web: 新增 `member_type === 'adult'` 分支，显示健康指标趋势看板（身高、体重、眼轴、视力）
  - 移动端: 新增 `isAdult` 条件判断，显示健康指标卡片
- **验证**: 代码已更新，需要部署后验证
- **UT**: 待添加
- **部署**: ✅ 已部署 (2026-04-05)
- **UT**: ✅ 已添加 `test_adult_member_trends.py`

---

## 📅 v2.9.1 血检指标功能期记录 (2026-04-05)

### [BUG-052] 移动端血检看板功能缺失
- **现象**: 成人/老人成员详情页缺少血检指标（血糖、血脂等）展示
- **根由**: 前端只处理了儿童类型的视力/生长发育看板
- **修复**:
  - 后端: 新增 `/members/{id}/blood-dashboard` API (血糖、总胆固醇、甘油三酯、高密度脂蛋白、低密度脂蛋白、血红蛋白、糖化血红蛋白)
  - 前端 Web: 成人/老人合并显示健康指标趋势 + 血检指标看板
  - 移动端: 成人/老人合并显示健康指标卡片 + 血检指标卡片
- **验证**: 后端 UT 通过
- **UT**: ✅ 已添加 `test_adult_member_trends.py` 验证 blood-dashboard API
- **部署**: ✅ APK 已安装

### [BUG-053] 移动端 Debug APK 白屏问题
- **现象**: 移动端打开后红字白屏
- **根由**: Debug APK 依赖 Metro 服务器，但打包时 bundle 未正确嵌入
- **修复**: 重新 export 并复制 bundle 到 assets 目录
- **验证**: APK 已重新构建安装
- **UT**: ✅ 全量回归 161 passed, 4 skipped
