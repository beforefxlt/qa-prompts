...
class ExamRecord(Base):
    ...
    baseline_age_months: Mapped[int] = mapped_column(Integer) # 基准月龄用于百分位查询
...
class Observation(Base):
    ...
    metric_code: Mapped[str] = mapped_column(String(50)) # height, weight, glucose, tc, tg, hdl, ldl, axial_length, vision_acuity
    value_numeric: Mapped[float] = mapped_column(Float)
    value_text: Mapped[Optional[str]] = mapped_column(String(255))
    unit: Mapped[str] = mapped_column(String(20))
    side: Mapped[Optional[str]] = mapped_column(String(10)) # left, right
